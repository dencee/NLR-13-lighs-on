...
function initializeGame() {
    ...
    for (let row = 0; row < NUM_ROWS; row++) {
        ...
        const eachRowEl = document.createElement('tr');
        ...
        for (let col = 0; col < NUM_COLS; col++) {
            ...
            eachRowEl.appendChild(eachTdEl);
        }

        // Add the table row to the gameGrid
        gameGrid.appendChild(eachRowEl);
    }

    intializeSetupButtons();
}

function intializeSetupButtons(){
    // 1. Select the button
    const randomButton = document.getElementById('randomize');

    // 2. Add the event listener
    // 3. Setup the callbackFn
...
function toggleLight(cellClicked){
    ...
    const currentBgColor = cellClicked.style.backgroundColor;

    // const LIGHTS_OFF_COLOR = 'gray';
    // const LIGHTS_ON_COLOR = 'orange';
    // If the background color is OFF, switch to ON
    if(currentBgColor === LIGHTS_OFF_COLOR){
